...
def forward_step(
    ...
    # else:
    #     moe_loss = 0.0
    if neox_args.aux_loss:
        moe_loss = neox_args.moe_loss_coeff * sum(moe_losses) / len(moe_losses)
        # modify the moe_loss below to add logics for other loss terms
    else:
        moe_loss = torch.tensor(0.0, device=main_loss.device, dtype=main_loss.dtype)

    loss = {"main_loss": main_loss, "moe_loss": moe_loss}
    if neox_args.memory_profiling:
        torch.cuda.nvtx.range_pop()
    if return_logits:
        return loss, outputs
...
def train_step(neox_args, timers, data_iterator, model, optimizer, lr_scheduler):
    ...
    if neox_args.is_pipe_parallel:
        ...
        for _ in range(neox_args.gradient_accumulation_steps):
            ...
            timers("forward").stop()
            main_losses.append(loss["main_loss"])
            moe_losses.append(loss["moe_loss"])
            ...
            backward_step(
                ...
                optimizer=optimizer,
                model=model,
                # loss=loss["main_loss"]+loss["moe_loss"],
            ...
            if (
                ...
                save_snapshot(neox_args)
        reduced_loss = {
            "moe_loss": reduce_losses(moe_losses).mean(),
            "lm_loss": reduce_losses(main_losses).mean(),
        }  # reduces main_losses across machines for logging
...
def evaluate(
    ...
    # reduces main_losses across processes for logging & run eval harness tasks
    eval_results = {
        "moe_loss": reduce_losses(moe_losses).mean().item(),
        "lm_loss": reduce_losses(main_losses).mean().item()
    }
