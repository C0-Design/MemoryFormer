...
if __name__ == "__main__":
    ...
    if args.test and torch.distributed.get_world_size() == 1:
        ...
        with torch.no_grad():
            ...
            b, seq_len = 2, 6
            inputs = torch.tensor(
                [[12092, 309, 717, 247, 15688, 15],[2811, 403, 368, 1469, 11608, 15]]
            ).cuda()
            mask = (
            ...
            for i in range(b):
                ...
                print('hflogits', outputs.logits[i].argmax(-1))
                print('neoxlogits', outputs_neox[i].argmax(-1))
