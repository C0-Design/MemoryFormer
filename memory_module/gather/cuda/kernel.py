...
class WeightedVectorGatherAdd(Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        """
        grad_weights = 
        grad_source = torch.zeros_like(source, dtype=source.dtype, device=source.device)
        for i in range(grad_source.shape[0]):
            grad_source[i] = (grad_outputs[torch.where(indexes==i)[0]]*weights[torch.where(indexes==i)].unsqueeze(-1)).sum(0)
        """
